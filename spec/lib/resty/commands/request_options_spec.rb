...
describe Resty::Commands::RequestOptions do
  let(:options) { Resty::Commands::RequestOptions.new("get /api/cats") }
  ...
  context "#data" do
    ...
    context "with data" do
      let(:options) { Resty::Commands::RequestOptions.new("get /api/cats #{data}") }
  ...
  context "#method_valid?" do
    it "returns true with valid methods" do
      %w{get put post delete head option patch}.each do |method|
        options = Resty::Commands::RequestOptions.new("#{method} /api/cats")
        expect(options.method_valid?).to be_true
      end
    end

    it "returns false" do
      options = Resty::Commands::RequestOptions.new("foo /api/cats")
  ...
  context "#path_valid?" do
    it "returns true" do
      options = Resty::Commands::RequestOptions.new("get /api/cats")
      expect(options.path_valid?).to be_true
    end

    it "returns false" do
      options = Resty::Commands::RequestOptions.new("get")
  ...
  context "#data_valid?" do
    it "returns true with data" do
      options = Resty::Commands::RequestOptions.new("get /api/cats #{data}")
  ...
  context "#valid?" do
    it "returns true" do
      expect(Resty::Commands::RequestOptions.new("get /api/cats #{data}").valid?).to be_true
    end

    it "returns false with invalid method" do
      expect(Resty::Commands::RequestOptions.new("foo /api/cats #{data}").valid?).to be_false
    end

    it "returns false with invalid path" do
      expect(Resty::Commands::RequestOptions.new("foo").valid?).to be_false
    end
